    ...
    pub fn run(config_metadata: &ConfigMetadata) -> (OptimizerResult, Vec<f32>, i32) {
        ...
        for epoch in 0..config_metadata.max_epochs {
            ...
            for dimmension in 0..config_metadata.derrivatives.len(){
                next[dimmension] = current[dimmension] - config_metadata.step_size * (config_metadata.derrivatives[dimmension])(current[dimmension]);
...
//             next = current - momentum;

//             let loss = next - current;

//             (config_metadata.epoch_printer)(epoch, current, loss, Some(momentum));
//             epochs += 1;
//             if loss.abs() <= config_metadata.precision {
//                 found = true;
//                 break;
